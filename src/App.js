  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        </select><button onClick={(e)=>{this.onRandom()}}>Random</button><br/>
        <label>{`Name: ${_.get(randomResult,"name","")}`}</label><br/>
        <label>{`Address: ${_.get(randomResult,"address","")}`}</label><br/>
        <label>{`Price range: ${_.get(randomResult,"price_range","")}`}</label><br/>
      </div>
    ); 
  }
}